...
void Path::InitLaneSegments()
{
    if (lane_segments_.empty())
    {
        for (int i = 0; i + 1 < num_points_; ++i)
        {
            ...
            if (FindLaneSegment(
                        path_points_[i], path_points_[i + 1], &lane_segment))
            {
...
void Path::InitWidth()
{
    ...
    for (int i = 0; i < num_sample_points_; ++i)
    {
        ...
        else
        {
            ...
            waypoint.lane->GetWidth(
                    waypoint.s, &lane_left_width, &lane_right_width);
            ...
            waypoint.lane->GetRoadWidth(
                    waypoint.s, &road_left_width, &road_right_width);
        ...
        for (const auto& overlap : GetOverlaps_from_lane(*(lane_segment.lane)))
        {
                    ...
                    lane_overlap_info.end_s() >= lane_segment.start_s)
            {
                ...
                const double adjusted_end_s =
                        std::min(
                                lane_overlap_info.end_s(), lane_segment.end_s) +
                        ref_s;
        ...
        for (const auto& segment : segments)
        {
            ...
            else
            {
                overlaps->emplace_back(
                        object_id, segment.first, segment.second);
            }
        }
    }
...
{
    auto next = std::upper_bound(lane_overlaps_.begin(), lane_overlaps_.end(),
            s, [](double s, const PathOverlap& o) { return s < o.start_s; });
...
void Path::InitOverlaps()
{
    ...
    GetAllOverlaps(
            std::bind(&LaneInfo::yield_signs, _1), &yield_sign_overlaps_);
    ...
    GetAllOverlaps(
            std::bind(&LaneInfo::pnc_junctions, _1), &pnc_junction_overlaps_);
    GetAllOverlaps(
            std::bind(&LaneInfo::clear_areas, _1), &clear_area_overlaps_);
    GetAllOverlaps(
            std::bind(&LaneInfo::speed_bumps, _1), &speed_bump_overlaps_);
    GetAllOverlaps(
            std::bind(&LaneInfo::parking_spaces, _1), &parking_space_overlaps_);
}
    ...
    if (std::abs(index.offset) > kMathEpsilon)
    {
        ...
        if (index.id < num_segments_ && !ref_point.lane_waypoints().empty())
        {
            ...
            if (lane_segment.lane != nullptr)
            {
                ...
                point.add_lane_waypoint(LaneWaypoint(lane_segment.lane,
                        lane_segment.start_s + index.offset,
                        ref_lane_waypoint.l));
            }
        }
        if (point.lane_waypoints().empty() &&
...
InterpolatedIndex Path::GetLaneIndexFromS(double s) const
{
    ...
    auto iter = std::lower_bound(
            lane_accumulated_s_.begin(), lane_accumulated_s_.end(), s);
...
bool Path::GetNearestPoint(
        const Vec2d& point, double* accumulate_s, double* lateral) const
{
    double distance = 0.0;
...
bool Path::GetProjectionWithHueristicParams(const Vec2d& point,
        const double hueristic_start_s, const double hueristic_end_s,
        ...
        double* lateral, double* min_distance) const
{
    ...
    {
        return approximation_.GetProjection(
                *this, point, accumulate_s, lateral, min_distance);
    }
    CHECK_GE(num_points_, 2);
    *min_distance = std::numeric_limits<double>::infinity();
    int min_index = 0;
...
bool Path::GetLaneWidth(
        const double s, double* lane_left_width, double* lane_right_width) const
{
...
bool Path::GetRoadWidth(
        const double s, double* road_left_width, double* road_right_width) const
{
...
double PathApproximation::compute_max_error(
        const Path& path, const int s, const int t)
{
    if (s + 1 >= t)
    {
        return 0.0;
...
bool PathApproximation::is_within_max_error(
        const Path& path, const int s, const int t)
{
    if (s + 1 >= t)
    {
...
bool PathApproximation::GetProjection(const Path& path,
        const common::math::Vec2d& point, double* accumulate_s, double* lateral,
    ...
    for (size_t i = 0; i < segments_.size(); ++i)
    {
        ...
        if (first_segment_idx < last_segment_idx)
        {
            ...
            if (min_projection > 0.0)
            {
                ...
                {
                    first_segment_idx = std::max(first_segment_idx,
...
bool PathApproximation::OverlapWith(
        const Path& path, const Box2d& box, double width) const
{
    if (num_points_ == 0)
    {
        return false;
