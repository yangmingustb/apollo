...
bool PncMap::UpdateRoutingResponse(const routing::RoutingResponse &routing)
{
    ...
    for (size_t j = 0; j < route_indices_.size(); ++j)
    {
        while (i < request_waypoints.size() &&
                RouteSegments::WithinLaneSegment(
                        route_indices_[j].segment, request_waypoints.Get(i)))
        {
...
int PncMap::SearchForwardWaypointIndex(
        int start, const LaneWaypoint &waypoint) const
{
...
int PncMap::SearchBackwardWaypointIndex(
        int start, const LaneWaypoint &waypoint) const
{
...
bool PncMap::PassageToSegments(
        routing::Passage passage, RouteSegments *segments) const
{
...
bool PncMap::GetNearestPointFromRouting(
        const VehicleState &state, LaneWaypoint *waypoint) const
{
    ...
    const int status = hdmap_->GetLanesWithHeading(point, kMaxDistance,
            state.heading(), M_PI / 2.0 + kHeadingBuffer, &lanes);
    ...
    if (valid_lanes.empty())
    {
        std::copy_if(lanes.begin(), lanes.end(),
                std::back_inserter(valid_lanes), [&](LaneInfoConstPtr ptr) {
...
bool PncMap::ExtendSegments(const RouteSegments &segments, double start_s,
        double end_s, RouteSegments *const truncated_segments) const
{
    if (segments.empty())
    {
    ...
    if (start_s < 0)
    {
        ...
        while (extend_s > kRouteEpsilon)
        {
            if (s <= kRouteEpsilon)
            {
                ...
                if (lane == nullptr || unique_lanes.find(lane->id().id()) !=
                                               unique_lanes.end())
                {
                    break;
                }
        ...
        truncated_segments->insert(truncated_segments->begin(),
                extended_lane_segments.rbegin(), extended_lane_segments.rend());
    }
    ...
    if (router_s < end_s && !truncated_segments->empty())
    {
        ...
        if (back.lane->total_length() > back.end_s)
        {
            ...
            back.end_s = std::min(
                    back.end_s + end_s - router_s, back.lane->total_length());
...
void PncMap::AppendLaneToPoints(LaneInfoConstPtr lane, const double start_s,
        const double end_s, std::vector<MapPathPoint> *const points)
{
