        ...
        const TrajectoryPoint& planning_start_point, Frame* frame)
{
    ...
    for (auto& reference_line_info : *frame->mutable_reference_line_info())
    {
        ...
        auto cur_status = PlanOnReferenceLine(
                planning_start_point, frame, &reference_line_info);
        ...
        ReferenceLineInfo* reference_line_info)
{
    ...
    if (reference_line_info->path_data().Empty())
    {
        ...
        GenerateFallbackPathProfile(
                reference_line_info, reference_line_info->mutable_path_data());
        ...
        const ReferenceLineInfo* reference_line_info, PathData* path_data)
{
    ...
    if (!reference_line.XYToSL(adc_point.path_point(), &adc_point_s_l))
    {
        ...
        {
            path_points.push_back(PointFactory::ToPathPoint(adc_traversed_x,
                    adc_traversed_y, 0.0, s, adc_point_heading, adc_point_kappa,
                    adc_point_dkappa));
    ...
    for (double s = adc_s; s < max_s; s += unit_s)
    {
        ...
        path_points.push_back(PointFactory::ToPathPoint(ref_point.x() + dx,
                ref_point.y() + dy, 0.0, s - adc_s, ref_point.heading(),
