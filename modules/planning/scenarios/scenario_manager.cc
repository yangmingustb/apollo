        ...
        const Frame& frame)
{
    ...
    if (traffic_sign_overlap && pnc_junction_overlap)
    {
        ...
        double s_diff = std::fabs(
                traffic_sign_overlap->start_s - pnc_junction_overlap->start_s);
        ...
        const Frame& frame)
{
    ...
    if (std::fabs(adc_speed) < max_abs_speed_when_stopped &&
            ...
            (HDMapUtil::BaseMap().GetNearestLaneWithHeading(adc_point, 2.0,
                     vehicle_state.heading(), M_PI / 3.0, &lane, &s, &l) != 0 ||
        ...
        const Frame& frame, const ScenarioConfig::ScenarioType& scenario_type)
{
    ...
    const std::vector<PathOverlap>& yield_sign_overlaps =
            reference_line_info.reference_line()
                    .map_path()
                    .yield_sign_overlaps();
    auto yield_sign_overlap_itr = std::find_if(yield_sign_overlaps.begin(),
            yield_sign_overlaps.end(),
