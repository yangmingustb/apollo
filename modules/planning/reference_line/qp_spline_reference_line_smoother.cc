        ...
        ReferenceLine* const smoothed_reference_line)
{
            ...
            ++i, t += resolution)
    {
        ...
        const double kappa = CurveMath::ComputeCurvature(spline.DerivativeX(t),
                spline.SecondDerivativeX(t), spline.DerivativeY(t),
...
bool QpSplineReferenceLineSmoother::Sampling()
{
    ...
    uint32_t num_spline = std::max(1u,
...
bool QpSplineReferenceLineSmoother::AddConstraint()
{
    ...
    for (const auto& point : anchor_points_)
    {
        ...
        xy_points.emplace_back(
                path_point.x() - ref_x_, path_point.y() - ref_y_);
    }
    ...
    if (FLAGS_enable_reference_line_stitching &&
            !spline_constraint->AddPointAngleConstraint(
                    evaluated_t.front(), headings.front()))
    {
