        ...
        ReferenceLine* const smoothed_reference_line)
{
    ...
    std::vector<common::PathPoint> smoothed_point2d = Interpolate(opt_theta,
            opt_kappa, opt_dkappa, opt_s, opt_x, opt_y, config_.resolution());
        ...
        std::vector<double>* ptr_y) const
{
    ...
    app->Options()->SetIntegerValue(
            "max_iter", config_.spiral().max_iteration());
    app->Options()->SetNumericValue("tol", config_.spiral().opt_tol());
    app->Options()->SetNumericValue(
            "acceptable_tol", config_.spiral().opt_acceptable_tol());
    ...
    ptop->get_optimization_results(
            ptr_theta, ptr_kappa, ptr_dkappa, ptr_s, ptr_x, ptr_y);
        ...
        std::vector<double>* ptr_x, std::vector<double>* ptr_y) const
{
    ...
    if (fixed_start_point_)
    {
        ptop->set_start_point(fixed_start_x_, fixed_start_y_,
                fixed_start_theta_, fixed_start_kappa_, fixed_start_dkappa_);
    }
        ...
        const double resolution) const
{
    ...
    double start_s = 0.0;
    common::PathPoint first_point = to_path_point(x.front(), y.front(), start_s,
            theta.front(), kappa.front(), dkappa.front());
    ...
    for (size_t i = 0; i + 1 < theta.size(); ++i)
    {
        ...
        auto path_point_seg = Interpolate(start_x, start_y, start_s, theta[i],
                kappa[i], dkappa[i], theta[i + 1], kappa[i + 1], dkappa[i + 1],
                s[i], resolution);
...
common::PathPoint SpiralReferenceLineSmoother::to_path_point(const double x,
        const double y, const double s, const double theta, const double kappa,
