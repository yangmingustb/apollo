    ...
    : vehicle_state_provider_(vehicle_state_provider)
{
    ...
    ACHECK(cyber::common::GetProtoFromFile(
            FLAGS_smoother_config_filename, &smoother_config_))
        ...
        std::list<hdmap::RouteSegments> *segments)
{
    ...
    if (FLAGS_enable_reference_line_provider_thread)
    {
        ...
        {
            reference_lines->assign(
                    reference_lines_.begin(), reference_lines_.end());
        ...
        std::list<hdmap::RouteSegments> *route_segments)
{
    ...
    while (iter != route_segments->end())
    {
        if (!iter->IsOnSegment())
        {
            route_segments->splice(
                    route_segments->begin(), *route_segments, iter);
            break;
        }
        ++iter;
    }
                        ...
                        left_neighbor_lane_ids.end(), target_lane_pair.first))
        {
            ...
            nearest_neighbor_lane_id = adc_lane_way_point.lane->lane()
                 ...
                 std::find(right_neighbor_lane_ids.begin(),
                         right_neighbor_lane_ids.end(), target_lane_pair.first))
        {
        ...
        std::list<hdmap::RouteSegments> *segments)
{
    ...
    {  // stitching reference line
        ...
        {
            reference_lines->emplace_back();
            if (!ExtendReferenceLine(
                        vehicle_state, &(*iter), &reference_lines->back()))
            {
        ...
        ReferenceLine *reference_line, RouteSegments *segments)
{
    ...
    if (need_shrink)
    {
        if (!reference_line->Segment(
                    sl.s(), new_backward_distance, new_forward_distance))
        {
        ...
        std::vector<AnchorPoint> *anchor_points) const
{
    ...
    common::util::uniform_slice(
            0.0, reference_line.Length(), num_of_anchors - 1, &anchor_s);
...
bool ReferenceLineProvider::SmoothRouteSegment(
        const RouteSegments &segments, ReferenceLine *reference_line)
{
