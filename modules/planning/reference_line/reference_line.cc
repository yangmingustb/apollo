...
bool ReferenceLine::Segment(
        const double s, const double look_backward, const double look_forward)
{
...
ReferenceLine::ToFrenetFrame(const common::TrajectoryPoint& traj_point) const
{
    ...
    CartesianFrenetConverter::cartesian_to_frenet(sl_point.s(), ref_point.x(),
            ref_point.y(), ref_point.heading(), ref_point.kappa(),
            ref_point.dkappa(), traj_point.path_point().x(),
...
double ReferenceLine::FindMinDistancePoint(const ReferencePoint& p0,
        const double s0, const ReferencePoint& p1, const double s1,
...
ReferencePoint ReferenceLine::GetReferencePoint(
        const double x, const double y) const
{
    ...
    double s =
            ReferenceLine::FindMinDistancePoint(reference_points_[index_start],
...
bool ReferenceLine::SLToXY(
        const SLPoint& sl_point, common::math::Vec2d* const xy_point) const
{
...
bool ReferenceLine::XYToSL(
        const common::math::Vec2d& xy_point, SLPoint* const sl_point) const
{
    double s = 0.0;
    double l = 0.0;
        ...
        const double s)
{
    ...
    return ReferencePoint(
            hdmap::MapPathPoint({x, y}, heading, waypoints), kappa, dkappa);
}
...
bool ReferenceLine::IsBlockRoad(
        const common::math::Box2d& box2d, double gap) const
{
        ...
        SLBoundary* const sl_boundary) const
{
    ...
    double l = 0.0;
    double distance = 0.0;
    if (!map_path_.GetProjectionWithHueristicParams(
                box.center(), start_s, end_s, &s, &l, &distance))
    {
...
bool ReferenceLine::GetSLBoundary(
        const hdmap::Polygon& polygon, SLBoundary* const sl_boundary) const
{
    double start_s(std::numeric_limits<double>::max());
    double end_s(std::numeric_limits<double>::lowest());
    double start_l(std::numeric_limits<double>::max());
...
double ReferenceLine::GetSpeedLimitFromS(const double s) const
{
    ...
    for (const auto& lane_waypoint : map_path_point.lane_waypoints())
    {
        ...
        speed_limit = std::fmin(
                lane_waypoint.lane->lane().speed_limit(), speed_limit);
    }
...
void ReferenceLine::AddSpeedLimit(
        double start_s, double end_s, double speed_limit)
{
        ...
        else
        {
            ...
            if (start_s >= limit.start_s)
            {
                ...
                if (end_s <= limit.end_s)
                {
                    ...
                    new_speed_limit.emplace_back(
                            end_s, limit.end_s, limit.speed_limit);
                }
                else
                {
                    new_speed_limit.emplace_back(
                            start_s, limit.end_s, min_speed);
                }
            }
            else
            {
                new_speed_limit.emplace_back(
                        start_s, limit.start_s, speed_limit);
                if (end_s <= limit.end_s)
                {
                    new_speed_limit.emplace_back(
                            limit.start_s, end_s, min_speed);
                    new_speed_limit.emplace_back(
                            end_s, limit.end_s, limit.speed_limit);
                }
                else
                {
                    new_speed_limit.emplace_back(
                            limit.start_s, limit.end_s, min_speed);
                }
            }
            start_s = limit.end_s;
