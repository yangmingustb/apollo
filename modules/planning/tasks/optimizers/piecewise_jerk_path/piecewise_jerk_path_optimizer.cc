        ...
        PathData* const final_path_data)
{
    ...
    /* why use front axe center? backward driving? */
    ...
    std::array<double, 5> w = {config.l_weight(),
    ...
    for (const auto& path_boundary : path_boundaries)
    {
        ...
        // if the path_boundary is lane following, it is possible to have less
        // than 2 points skip path boundary of this kind
        ...
        if (!FLAGS_enable_force_pull_over_open_space_parking_test)
        {
                    ...
                    path_boundary.label().find("pullover") != std::string::npos)
            {
                ...
                reference_line.XYToSL(
                        pull_over_status.position(), &pull_over_sl);
        ...
        size_t path_reference_size =
                reference_path_data.path_reference().size();
        ...
        for (size_t i = 0; i < path_boundary_size; ++i)
        {
            ...
            ddl_bounds.emplace_back(
                    -lat_acc_bound - kappa, lat_acc_bound - kappa);
        }

        bool res_opt = OptimizePath(init_frenet_state.second, end_state,
                std::move(path_reference_l), path_reference_size,
        ...
        if (res_opt)
        {
            ...
            auto frenet_frame_path = ToPiecewiseJerkPath(opt_l, opt_dl, opt_ddl,
                    path_boundary.delta_s(), path_boundary.start_s());
...
// how to add boundary constraints and obstalce avoidance constraints?
        ...
        std::vector<double>* ddx)
{
    ...
    // if path ref is invalid, exclude?
    ...
    if (is_valid_path_reference)
    {
        ...
        piecewise_jerk_problem.set_x_ref(
                std::move(weight_x_ref_vec), std::move(path_reference_l_ref));
    }
    ...
    const double jerk_bound = EstimateJerkBoundary(
            std::fmax(init_state[1], 1.0), axis_distance, max_yaw_rate);
        ...
        const double start_s) const
{
    ...
    PiecewiseJerkTrajectory1d piecewise_jerk_traj(
            x.front(), dx.front(), ddx.front());
    ...
    /* why add evaluate function for l, dl, dl, ddl */
...
double PiecewiseJerkPathOptimizer::GaussianWeighting(const double x,
        const double peak_weighting, const double peak_weighting_x) const
{
