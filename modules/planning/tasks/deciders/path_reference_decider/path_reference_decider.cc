...
Status PathReferenceDecider::Execute(
        ...
        Frame *frame, ReferenceLineInfo *reference_line_info)
{
    ...
    ConvertTrajectoryToPath(
            stitched_learning_model_trajectory, &path_reference);
    ...
    if (!IsValidPathReference(*reference_line_info,
                path_boundaries[regular_path_bound_idx], path_reference))
    {
        reference_line_info->mutable_path_data()->set_is_valid_path_reference(
                false);
        ADEBUG << "valid_path_reference_counter["
        ...
        std::vector<std::vector<LineSegment2d>> *path_bound_segments)
{
    ...
    reference_line_info_->reference_line().SLToXY(
            prev_left_sl_point, &prev_left_cartesian_point);
    reference_line_info_->reference_line().SLToXY(
            prev_right_sl_point, &prev_right_cartesian_point);
    for (size_t i = 1; i < path_bound_size; ++i)
    {
        ...
        reference_line_info_->reference_line().SLToXY(
                left_sl_point, &left_cartesian_point);
        reference_line_info_->reference_line().SLToXY(
                right_sl_point, &right_cartesian_point);

        // together with previous point form line segment
        LineSegment2d cur_left_segment(
                prev_left_cartesian_point, left_cartesian_point);
        LineSegment2d cur_right_segment(
        ...
        const PathBoundary &path_bound, const double x, const double y)
{
    ...
    else
    {
        ...
        if (std::get<0>(path_bound.boundary().at(idx_before)) <= point_sl.l() &&
                std::get<1>(path_bound.boundary().at(idx_before)) >=
                        point_sl.l() &&
                std::get<0>(path_bound.boundary().at(idx_after)) <=
                        point_sl.l() &&
                std::get<1>(path_bound.boundary().at(idx_after)) >=
                        point_sl.l())
        {
            return idx_after;
        }
