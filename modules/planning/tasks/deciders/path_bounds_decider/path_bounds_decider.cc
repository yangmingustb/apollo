        ...
        Frame* const frame, ReferenceLineInfo* const reference_line_info)
{
    ...
    Status ret = GenerateFallbackPathBound(
    ...
    if (plan_pull_over_path)
    {
        ...
        Status ret = GeneratePullOverPathBound(
                *frame, *reference_line_info, &pull_over_path_bound);
        if (!ret.ok())
        {
    ...
    for (const auto& lane_borrow_info : lane_borrow_info_list)
    {
        ...
        Status ret = GenerateRegularPathBound(*reference_line_info,
                lane_borrow_info, &regular_path_bound, &blocking_obstacle_id,
                &borrow_lane_type);
        if (!ret.ok())
        {
            continue;
...
void
PathBoundsDecider::InitPathBoundsDecider(
        const Frame& frame, const ReferenceLineInfo& reference_line_info)
{
    ...
    if (!reference_line.GetLaneWidth(
                adc_frenet_s_, &lane_left_width, &lane_right_width))
    {
...
PathBoundsDecider::InferFrontAxeCenterFromRearAxeCenter(
...
Status
PathBoundsDecider::GenerateLaneChangePathBound(
        const ReferenceLineInfo& reference_line_info,
...
Status
PathBoundsDecider::GeneratePullOverPathBound(const Frame& frame,
        const ReferenceLineInfo& reference_line_info,
        PathBound* const path_bound)
{
    ...
    {
        const std::string msg =
                "ADC is outside road boundary already. Cannot generate "
                "pull-over path";
        AERROR << msg;
        return Status(ErrorCode::PLANNING_ERROR, msg);
    }
    ...
    {
        curr_idx = IsPointWithinPathBound(reference_line_info,
...
Status
PathBoundsDecider::GenerateFallbackPathBound(
        const ReferenceLineInfo& reference_line_info,
        PathBound* const path_bound)
{
    // 1. Initialize the path boundaries to be an indefinitely large area.
...
int
PathBoundsDecider::IsPointWithinPathBound(
...
bool
PathBoundsDecider::FindDestinationPullOverS(const Frame& frame,
        const ReferenceLineInfo& reference_line_info,
        const std::vector<std::tuple<double, double, double>>& path_bound,
        double* pull_over_s)
{
...
bool
PathBoundsDecider::FindEmergencyPullOverS(
...
bool
PathBoundsDecider::SearchPullOverPosition(const Frame& frame,
        const ReferenceLineInfo& reference_line_info,
        const std::vector<std::tuple<double, double, double>>& path_bound,
        std::tuple<double, double, double, int>* const pull_over_configuration)
{
    ...
    {
        if (!FindDestinationPullOverS(
                    frame, reference_line_info, path_bound, &pull_over_s))
        {
                            ...
                            pull_over_space_length))
    {
        ...
        if (is_feasible_window)
        {
            ...
            double s = 0.0;
            double l = 0.0;
            auto point = common::util::PointFactory::ToPointENU(
                    pull_over_x, pull_over_y);
            if (HDMapUtil::BaseMap().GetNearestLaneWithHeading(point, 5.0,
                        pull_over_theta, M_PI_2, &lane, &s, &l) == 0)
            {
...
void
PathBoundsDecider::RemoveRedundantPathBoundaries(
...
bool
PathBoundsDecider::InitPathBoundary(
        const ReferenceLineInfo& reference_line_info,
        PathBound* const path_bound)
{
    // Sanity checks.
...
bool
PathBoundsDecider::GetBoundaryFromRoads(
        const ReferenceLineInfo& reference_line_info,
        PathBound* const path_bound)
{
    // Sanity checks.
...
bool
PathBoundsDecider::GetBoundaryFromLanes(
        const ReferenceLineInfo& reference_line_info,
        const LaneBorrowInfo& lane_borrow_info, PathBound* const path_bound,
        std::string* const borrow_lane_type)
{
    ...
    for (size_t i = 0; i < path_bound->size(); ++i)
    {
        ...
        if (CheckLaneBoundaryType(
                    reference_line_info, curr_s, lane_borrow_info))
        {
...
bool
PathBoundsDecider::GetBoundaryFromADC(
...
bool
PathBoundsDecider::GetBoundaryFromLanesAndADC(
        const ReferenceLineInfo& reference_line_info,
        ...
        bool is_fallback_lanechange)
{
    ...
    for (size_t i = 0; i < path_bound->size(); ++i)
    {
        ...
        if (!UpdatePathBoundaryWithBuffer(i, curr_left_bound, curr_right_bound,
                    path_bound, is_left_lane_boundary, is_right_lane_boundary))
        {
            path_blocked_idx = static_cast<int>(i);
        }
        if (path_blocked_idx != -1)
...
void
PathBoundsDecider::UpdatePullOverBoundaryByLaneBoundary(
        const ReferenceLineInfo& reference_line_info,
        PathBound* const path_bound)
{
    const ReferenceLine& reference_line = reference_line_info.reference_line();
...
void
PathBoundsDecider::ConvertBoundarySAxisFromLaneCenterToRefLine(
        const ReferenceLineInfo& reference_line_info,
        PathBound* const path_bound)
{
    const ReferenceLine& reference_line = reference_line_info.reference_line();
...
void
PathBoundsDecider::GetBoundaryFromLaneChangeForbiddenZone(
        const ReferenceLineInfo& reference_line_info,
        PathBound* const path_bound)
{
    // Sanity checks.
    ...
    if (lane_change_status->exist_lane_change_start_position())
    {
        ...
        reference_line.XYToSL(
                lane_change_status->lane_change_start_position(), &point_sl);
...
bool
PathBoundsDecider::GetBoundaryFromStaticObstacles(
                ...
                std::get<1>(sorted_obstacles[obs_idx]) < curr_s)
        {
                    ...
                    std::get<1>(sorted_obstacles[obs_idx]) < curr_s)
            {
                ...
                if (std::get<0>(curr_obstacle) == 1)
                {
                    ...
                    if (!UpdatePathBoundaryAndCenterLineWithBuffer(i,
                                *left_bounds.begin(), *right_bounds.begin(),
...
std::vector<PathBound>
PathBoundsDecider::ConstructSubsequentPathBounds(
        ...
        PathBound* const curr_path_bounds)
{
            ...
            std::get<1>(sorted_obstacles[obs_idx]) > curr_s)
    {
        ...
        bool is_able_to_update = UpdatePathBoundaryWithBuffer(path_idx,
                left_bounds_from_obstacles, right_bounds_from_obstacles,
                curr_path_bounds);
        ...
        if (is_able_to_update)
        {
            ret = ConstructSubsequentPathBounds(sorted_obstacles, path_idx + 1,
                    obs_idx, obs_id_to_details, curr_path_bounds);
        }
        else
        {
        ...
        for (size_t j = 0; j < pass_direction_decisions[i].size(); ++j)
        {
            if (pass_direction_decisions[i][j])
            {
                ...
                (*obs_id_to_details)[std::get<4>(new_entering_obstacles[j])] =
                        std::make_tuple(
                                true, std::get<3>(new_entering_obstacles[j]));
            }
            else
            {
                // Pass from right.
                (*obs_id_to_details)[std::get<4>(new_entering_obstacles[j])] =
                        std::make_tuple(
                                false, std::get<2>(new_entering_obstacles[j]));
            }
        }
...
PathBoundsDecider::DecidePassDirections(double l_min, double l_max,
...
double
PathBoundsDecider::GetBufferBetweenADCCenterAndEdge()
{
...
bool
PathBoundsDecider::UpdatePathBoundaryWithBuffer(size_t idx, double left_bound,
        double right_bound, PathBound* const path_boundaries,
        bool is_left_lane_bound, bool is_right_lane_bound)
{
    ...
    double left_adc_buffer_coeff =
            (is_left_lane_bound ? config_.path_bounds_decider_config()
                                          .adc_buffer_coeff()
                                : default_adc_buffer_coeff);
    double right_adc_buffer_coeff =
            (is_right_lane_bound ? config_.path_bounds_decider_config()
                                           .adc_buffer_coeff()
                                 : default_adc_buffer_coeff);

    // Update the right bound (l_min):
    double new_l_min = std::fmax(std::get<1>((*path_boundaries)[idx]),
            right_bound + right_adc_buffer_coeff *
                                  GetBufferBetweenADCCenterAndEdge());
    // Update the left bound (l_max):
    double new_l_max = std::fmin(std::get<2>((*path_boundaries)[idx]),
            left_bound -
                    left_adc_buffer_coeff * GetBufferBetweenADCCenterAndEdge());
...
bool
PathBoundsDecider::UpdatePathBoundaryAndCenterLineWithBuffer(size_t idx,
        double left_bound, double right_bound, PathBound* const path_boundaries,
...
bool
PathBoundsDecider::UpdatePathBoundary(size_t idx, double left_bound,
        double right_bound, PathBound* const path_boundaries)
{
    // Update the right bound (l_min):
    double new_l_min =
...
void
PathBoundsDecider::TrimPathBounds(
        const int path_blocked_idx, PathBound* const path_boundaries)
{
    if (path_blocked_idx != -1)
    {
...
void
PathBoundsDecider::PathBoundsDebugString(const PathBound& path_boundaries)
{
...
bool
PathBoundsDecider::CheckLaneBoundaryType(
...
void
PathBoundsDecider::RecordDebugInfo(const PathBound& path_boundaries,
