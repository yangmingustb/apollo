...
private:
     ...
     */
    voidMergeWithMainStop InitPathBoundsDecider(
    ...
    common::Status GeneratePullOverPathBound(const Frame& frame,
            const ReferenceLineInfo& reference_line_info,
            std::vector<std::tuple<double, double, double>>* const path_bound);
    ...
    bool SearchPullOverPosition(const Frame& frame,
            const ReferenceLineInfo& reference_line_info,
            const std::vector<std::tuple<double, double, double>>& path_bound,
    ...
    std::vector<std::vector<bool>> DecidePassDirections(double l_min,
            double l_max,
            const std::vector<std::tuple<int, double, double, double,
     ...
     *  @return If path is good, true; if path is blocked, false.
     */
    bool UpdatePathBoundaryAndCenterLineWithBuffer(size_t idx,
    ...
    bool CheckLaneBoundaryType(const ReferenceLineInfo& reference_line_info,
            const double check_s, const LaneBorrowInfo& lane_borrow_info);
