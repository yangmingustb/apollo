...
Status
OpenSpaceRoiDecider::Process(Frame *frame)
{
    if (frame == nullptr)
    {
    ...
    if (roi_type == OpenSpaceRoiDeciderConfig::PARKING)
    {
        ...
        if (!GetParkingBoundary(
                    frame, spot_vertices, nearby_path, &roi_boundary))
        {
            const std::string msg = "Fail to get parking boundary from map";
            AERROR << msg;
            return Status(ErrorCode::PLANNING_ERROR, msg);
...
// get origin from ADC
void
OpenSpaceRoiDecider::SetOriginFromADC(
...
void
OpenSpaceRoiDecider::SetParkingSpotEndPose(
        Frame *const frame, const std::array<common::math::Vec2d, 4> &vertices)
{
    auto left_top = vertices[0];
...
void
OpenSpaceRoiDecider::SetPullOverSpotEndPose(Frame *const frame)
{
...
void
OpenSpaceRoiDecider::SetParkAndGoEndPose(Frame *const frame)
{
...
void
OpenSpaceRoiDecider::GetRoadBoundary(const hdmap::Path &nearby_path,
        const double center_line_s, const common::math::Vec2d &origin_point,
        ...
        std::vector<double> *right_lane_road_width)
{
    ...
    while (check_point_s <= end_s)
    {
        ...
        AddBoundaryKeyPoint(nearby_path, check_point_s, start_s, end_s,
                ...
                left_lane_boundary, center_lane_s_left, left_lane_road_width);
...
void
OpenSpaceRoiDecider::GetRoadBoundaryFromMap(const hdmap::Path &nearby_path,
        ...
        std::vector<double> *right_lane_road_width)
{
    ...
    while (check_point_s <= end_s)
    {
        ...
        if (check_point_s < center_line_s)
        {
            for (size_t i = 0;
                    i <
                    (*road_boundaries.at(0)).left_boundary.line_points.size();
                    i++)
            {
            ...
            for (size_t i = 0;
                    i <
                    (*road_boundaries.at(0)).right_boundary.line_points.size();
                    i++)
            {
    ...
    if (!left_lane_boundary->empty())
    {
        ...
        auto unique_end = std::unique(
                left_lane_boundary->begin(), left_lane_boundary->end());
    ...
    if (!right_lane_boundary->empty())
    {
        ...
        auto unique_end = std::unique(
                right_lane_boundary->begin(), right_lane_boundary->end());
        right_lane_boundary->erase(unique_end, right_lane_boundary->end());
    }
}

void
OpenSpaceRoiDecider::AddBoundaryKeyPoint(const hdmap::Path &nearby_path,
        const double check_point_s, const double start_s, const double end_s,
        ...
        std::vector<double> *center_lane_s, std::vector<double> *road_width)
{
    ...
    double previous_road_width =
            is_left_curb ? nearby_path.GetRoadLeftWidth(
                                   check_point_s - previous_distance_s)
...
bool
OpenSpaceRoiDecider::GetParkingBoundary(Frame *const frame,
        const std::array<Vec2d, 4> &vertices, const hdmap::Path &nearby_path,
                ...
                *const roi_parking_boundary)
{
    ...
    if (!(nearby_path.GetProjection(left_top, &left_top_s, &left_top_l) &&
                nearby_path.GetProjection(
                        right_top, &right_top_s, &right_top_l)))
    {
    ...
    if (average_l < 0)
    {
        ...
        std::vector<Vec2d> parking_spot_boundary{
        ...
        std::copy(point_right_to_right_top_connor_itr,
                right_lane_boundary.end(), std::back_inserter(boundary_points));
        ...
        for (size_t i = 0; i < point_left_to_left_top_connor_index; i++)
        {
            std::vector<Vec2d> segment{
                    right_lane_boundary[i], right_lane_boundary[i + 1]};
        ...
        std::vector<Vec2d> right_stitching_segment{right_top,
        ...
        {
            std::vector<Vec2d> segment{
                    left_lane_boundary[i], left_lane_boundary[i - 1]};
            roi_parking_boundary->push_back(segment);
        }
    }
    else
    {
        ...
        std::reverse_copy(point_left_to_left_top_connor_itr,
                left_lane_boundary.end(), std::back_inserter(boundary_points));
    ...
    // Get xy boundary
    auto xminmax = std::minmax_element(boundary_points.begin(),
            boundary_points.end(),
            [](const Vec2d &a, const Vec2d &b) { return a.x() < b.x(); });
    auto yminmax = std::minmax_element(boundary_points.begin(),
            boundary_points.end(),
...
bool
OpenSpaceRoiDecider::GetPullOverBoundary(Frame *const frame,
...
bool
OpenSpaceRoiDecider::GetParkAndGoBoundary(Frame *const frame,
        const hdmap::Path &nearby_path,
        std::vector<std::vector<common::math::Vec2d>>
                *const roi_parking_boundary)
{
...
bool
OpenSpaceRoiDecider::GetParkingSpot(
        Frame *const frame, std::array<Vec2d, 4> *vertices, Path *nearby_path)
{
    if (frame == nullptr)
    {
    ...
    {
        int status = HDMapUtil::BaseMap().GetNearestLaneWithHeading(point, 10.0,
                vehicle_state_.heading(), M_PI / 2.0, &nearest_lane,
    ...
    std::array<Vec2d, 4> parking_vertices{
            left_top, left_down, right_down, right_top};
...
bool
OpenSpaceRoiDecider::GetPullOverSpot(Frame *const frame,
        std::array<common::math::Vec2d, 4> *vertices, hdmap::Path *nearby_path)
{
    const auto &pull_over_status =
            injector_->planning_context()->planning_status().pull_over();
    ...
    *nearby_path =
    ...
    std::array<Vec2d, 4> pull_over_vertices{
            left_top, left_down, right_down, right_top};
...
void
OpenSpaceRoiDecider::SearchTargetParkingSpotOnPath(
        const hdmap::Path &nearby_path,
...
bool
OpenSpaceRoiDecider::CheckDistanceToParkingSpot(const hdmap::Path &nearby_path,
        const hdmap::ParkingSpaceInfoConstPtr &target_parking_spot)
{
    ...
    nearby_path.GetNearestPoint(
            left_bottom_point, &left_bottom_point_s, &left_bottom_point_l);
    nearby_path.GetNearestPoint(
            right_bottom_point, &right_bottom_point_s, &right_bottom_point_l);
    Vec2d vehicle_vec(vehicle_state_.x(), vehicle_state_.y());
    nearby_path.GetNearestPoint(
            vehicle_vec, &vehicle_point_s, &vehicle_point_l);
...
bool
OpenSpaceRoiDecider::FuseLineSegments(
        std::vector<std::vector<common::math::Vec2d>> *line_segments_vec)
{
    ...
    while (cur_segment != line_segments_vec->end() - 1)
    {
                    ...
                    next_second_point) < 0.0)
        {
            ...
            next_segment->erase(
                    next_segment->begin(), next_segment->begin() + 2);
...
bool
OpenSpaceRoiDecider::FormulateBoundaryConstraints(
...
bool
OpenSpaceRoiDecider::LoadObstacleInVertices(
        const std::vector<std::vector<common::math::Vec2d>>
                &roi_parking_boundary,
        Frame *const frame)
{
    ...
    mutable_open_space_info->set_obstacles_num(
            parking_boundaries_num + perception_obstacles_num);
    return true;
}

bool
OpenSpaceRoiDecider::FilterOutObstacle(
...
bool
OpenSpaceRoiDecider::LoadObstacleInHyperPlanes(Frame *const frame)
{
...
bool
OpenSpaceRoiDecider::GetHyperPlanes(const size_t &obstacles_num,
...
bool
OpenSpaceRoiDecider::IsInParkingLot(const double adc_init_x,
        const double adc_init_y, const double adc_init_heading,
        std::array<Vec2d, 4> *parking_lot_vertices)
{
    ...
    ADEBUG << hdmap_;
    ADEBUG << hdmap_->GetParkingSpaces(
            adc_parking_spot, kDistance, &parking_lots);
...
void
OpenSpaceRoiDecider::GetParkSpotFromMap(
