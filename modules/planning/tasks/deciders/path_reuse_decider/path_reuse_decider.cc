...
/* 路径什么时候可以重用？为什么要reuse
LANE_FOLLOW_SCENARIO

*/
Status PathReuseDecider::Process(
        Frame* const frame, ReferenceLineInfo* const reference_line_info)
{
    ...
    // active path reuse during change_lane only, why path reuse in lane change?
...
void PathReuseDecider::GetADCSLPoint(
        const ReferenceLine& reference_line, common::SLPoint* adc_position_sl)
{
        ...
        ReferenceLineInfo* const reference_line_info)
{
    ...
    common::math::Vec2d path_end_position = {
            history_path.back().x(), history_path.back().y()};
    ...
    for (size_t i = 0; i < history_path.size(); ++i)
    {
        ...
        common::math::Vec2d path_position = {
