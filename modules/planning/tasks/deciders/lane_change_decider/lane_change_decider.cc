        ...
        Frame* frame, ReferenceLineInfo* const current_reference_line_info)
{
    ...
    {  
        // has lane to change in reference lines. why this api can get path id.
        ...
        else if (prev_status->status() == ChangeLaneStatus::CHANGE_LANE_FAILED)
        {
            ...
            {
                UpdateStatus(
                        now, ChangeLaneStatus::IN_CHANGE_LANE, current_path_id);
...
void LaneChangeDecider::UpdatePreparationDistance(const bool is_opt_succeed,
        const Frame* frame, const ReferenceLineInfo* const reference_line_info,
        ...
        std::list<ReferenceLineInfo>* reference_line_info) const
{
    ...
    reference_line_info->splice(
            reference_line_info->begin(), *reference_line_info, iter);
        ...
        const double is_block_angle_threshold)
{
            ...
            reference_line_info.path_decision().obstacles().Items())
    {
        ...
        if (std::fabs(common::math::NormalizeAngle(
                    left_most_angle - right_most_angle)) >
