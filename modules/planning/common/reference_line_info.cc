...
bool ReferenceLineInfo::Init(const std::vector<const Obstacle*>& obstacles)
{
    ...
    for (const auto& speed_bump : map_path.speed_bump_overlaps())
    {
        ...
        reference_line_.AddSpeedLimit(speed_bump.start_s - 1.0,
                speed_bump.end_s + 1.0, FLAGS_speed_bump_speed_limit);
    }
...
void ReferenceLineInfo::InitFirstOverlaps()
{
    ...
    if (GetFirstOverlap(
                map_path.pnc_junction_overlaps(), &pnc_junction_overlap))
    {
...
Obstacle* ReferenceLineInfo::AddObstacle(const Obstacle* obstacle)
{
    ...
    if (!reference_line_.GetSLBoundary(
                obstacle->PerceptionBoundingBox(), &perception_sl))
    {
    ...
    if (IsIrrelevantObstacle(*mutable_obstacle))
    {
        ...
        path_decision_.AddLateralDecision(
                "reference_line_filter", obstacle->Id(), ignore);
        path_decision_.AddLongitudinalDecision(
                "reference_line_filter", obstacle->Id(), ignore);
    ...
    if (FLAGS_use_multi_thread_to_add_obstacles)
    {
        ...
        for (const auto* obstacle : obstacles)
        {
            results.push_back(cyber::Async(
                    &ReferenceLineInfo::AddObstacle, this, obstacle));
        }
...
bool ReferenceLineInfo::CombinePathAndSpeedProfile(const double relative_time,
        const double start_s, DiscretizedTrajectory* ptr_discretized_trajectory)
{
    ...
    for (double cur_rel_time = 0.0; cur_rel_time < speed_data_.TotalTime();
            cur_rel_time +=
            (cur_rel_time < kDenseTimeSec ? kDenseTimeResoltuion
        ...
        DiscretizedTrajectory* adjusted_trajectory)
{
    ...
    for (size_t i = 0; i < trajectory.size(); ++i)
    {
        ...
        const double tracking_heading = std::atan2(
                cur_point_y - start_point_y, cur_point_x - start_point_x);
    ...
    cut_trajectory.erase(
            cut_trajectory.begin(), cut_trajectory.begin() + insert_idx);
