        ...
        const std::list<hdmap::RouteSegments> &segments)
{
    ...
    if (reference_line_info_.size() == 2)
    {
        ...
        if (!reference_line_info_.front().reference_line().XYToSL(
                    xy_point, &first_sl))
        {
            return false;
        }
        common::SLPoint second_sl;
        if (!reference_line_info_.back().reference_line().XYToSL(
                    xy_point, &second_sl))
        {
            return false;
        }
        ...
        const std::string &lane_id, const double lane_s)
{
    ...
    Box2d stop_wall_box{{dest_point.x(), dest_point.y()},
            lane->Heading(dest_lane_s), FLAGS_virtual_stop_wall_length,
        ...
        const double obstacle_end_s)
{
    ...
    if (!reference_line.GetLaneWidth(
                obstacle_start_s, &left_lane_width, &right_lane_width))
    {
