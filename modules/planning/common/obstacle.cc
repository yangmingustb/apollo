              ...
              perception_obstacle_.width())
{
    ...
    ACHECK(common::math::Polygon2d::ComputeConvexHull(
            polygon_points, &perception_polygon_))
        ...
        const double relative_time) const
{
    ...
    else
    {
        ...
        auto it_lower = std::lower_bound(
                points.begin(), points.end(), relative_time, comp);
        ...
        const prediction::PredictionObstacles& predictions)
{
    ...
    for (const auto& prediction_obstacle : predictions.prediction_obstacle())
    {
        ...
        {
            obstacles.emplace_back(new Obstacle(perception_id,
                    prediction_obstacle.perception_obstacle(),
        ...
        for (const auto& trajectory : prediction_obstacle.trajectory())
        {
            ...
            obstacles.emplace_back(new Obstacle(obstacle_id,
                    prediction_obstacle.perception_obstacle(), trajectory,
                    prediction_obstacle.priority().priority(),
                    prediction_obstacle.is_static()));
            ++trajectory_index;
        }
        ...
        const std::string& id, const common::math::Box2d& obstacle_box)
{
    ...
    auto* obstacle = new Obstacle(
            id, perception_obstacle, ObstaclePriority::NORMAL, true);
        ...
        const ReferenceLine& reference_line, const double adc_start_s)
{
    ...
    {
        if (BuildTrajectoryStBoundary(
                    reference_line, adc_start_s, &reference_line_st_boundary_))
        {
        ...
        const double adc_start_s, STBoundary* const st_boundary)
{
    ...
    for (int i = 1; i < trajectory_points.size(); ++i)
    {
        ...
        common::math::Box2d object_moving_box(center, first_point.theta(),
                object_moving_box_length, object_width);
        ...
        if (!reference_line.GetApproximateSLBoundary(
                    object_moving_box, start_s, end_s, &object_boundary))
        {
        ...
        if (!IsCautionLevelObstacle() &&
                (std::fmin(object_boundary.start_l(), object_boundary.end_l()) >
                                skip_l_distance ||
                        std::fmax(object_boundary.start_l(),
                                object_boundary.end_l()) < -skip_l_distance))
        {
            continue;
        }
        ...
        bool has_low = false;
        double high_s = std::min(
                object_boundary.end_s() + adc_half_length, FLAGS_st_max_s);
        bool has_high = false;
        while (low_s + st_boundary_delta_s < high_s && !(has_low && has_high))
        {
            if (!has_low)
            {
                ...
                has_low = object_moving_box.HasOverlap({low_ref,
                        low_ref.heading(), adc_length,
            ...
            if (!has_high)
            {
                ...
                has_high = object_moving_box.HasOverlap({high_ref,
                        high_ref.heading(), adc_length,
                        adc_width + FLAGS_nonstatic_obstacle_nudge_l_buffer});
        ...
        const ObjectDecisionType& lhs, const ObjectDecisionType& rhs)
{
    ...
    const auto lhs_val = FindOrDie(
            s_longitudinal_decision_safety_sorter_, lhs.object_tag_case());
    const auto rhs_val = FindOrDie(
            s_longitudinal_decision_safety_sorter_, rhs.object_tag_case());
    if (lhs_val < rhs_val)
    {
        return rhs;
    }
...
void Obstacle::CheckLaneBlocking(const ReferenceLine& reference_line)
{
    ...
    if (reference_line.IsOnLane(sl_boundary_) &&
            driving_width < vehicle_param.width() +
