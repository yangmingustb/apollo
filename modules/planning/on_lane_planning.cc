...
Status
OnLanePlanning::InitFrame(const uint32_t sequence_num,
        ...
        const VehicleState& vehicle_state)
{
    ...
    {
        return Status(
                ErrorCode::PLANNING_ERROR, "Fail to init frame: nullptr.");
    }

    std::list<ReferenceLine> reference_lines;
    std::list<hdmap::RouteSegments> segments;
    if (!reference_line_provider_->GetReferenceLines(
    ...
    auto status = frame_->Init(injector_->vehicle_state(), reference_lines,
...
void
OnLanePlanning::GenerateStopTrajectory(ADCTrajectory* ptr_trajectory_pb)
{
        ...
        const LocalView& local_view, ADCTrajectory* const ptr_trajectory_pb)
{
    ...
    std::vector<TrajectoryPoint> stitching_trajectory =
            TrajectoryStitcher::ComputeStitchingTrajectory(vehicle_state,
    ...
    else
    {
        ...
        ref_line_task->set_time_ms(
                reference_line_provider_->LastTimeDelay() * 1000.0);
...
void
OnLanePlanning::ExportReferenceLineDebug(planning_internal::Debug* debug)
{
    if (!FLAGS_enable_record_debug)
    {
        return;
...
Status
OnLanePlanning::Plan(const double current_time_stamp,
        ...
        ADCTrajectory* const ptr_trajectory_pb)
{
    ...
    auto status = planner_->Plan(
            stitching_trajectory.back(), frame_.get(), ptr_trajectory_pb);
    ...
    if (frame_->open_space_info().is_on_open_space_trajectory())
    {
        ...
        if (FLAGS_enable_record_debug)
        {
            ...
            ExportOpenSpaceChart(
                    ptr_trajectory_pb->debug(), *ptr_trajectory_pb, ptr_debug);
        }
    }
    else
    {
        ...
        else
        {
            ...
            if (failed_ref_info)
            {
                ExportFailedLaneChangeSTChart(
                        failed_ref_info->debug(), ptr_debug);
            }
        }
...
bool
OnLanePlanning::CheckPlanningConfig(const PlanningConfig& config)
{
...
void
PopulateChartOptions(double x_min, double x_max, std::string x_label,
        double y_min, double y_max, std::string y_label, bool display,
        Chart* chart)
{
...
void
AddSTGraph(const STGraphDebug& st_graph, Chart* chart)
{
    ...
    PopulateChartOptions(
            -2.0, 10.0, "t (second)", -10.0, 220.0, "s (meter)", false, chart);
...
void
AddSLFrame(const SLFrameDebug& sl_frame, Chart* chart)
{
...
void
AddSpeedPlan(const ::google::protobuf::RepeatedPtrField<SpeedPlan>& speed_plans,
        Chart* chart)
{
...
void
OnLanePlanning::ExportFailedLaneChangeSTChart(
...
void
OnLanePlanning::ExportOnLaneChart(const planning_internal::Debug& debug_info,
        planning_internal::Debug* debug_chart)
{
    const auto& src_data = debug_info.planning_data();
    auto* dst_data = debug_chart->mutable_planning_data();
...
void
OnLanePlanning::ExportOpenSpaceChart(const planning_internal::Debug& debug_info,
...
void
OnLanePlanning::AddOpenSpaceOptimizerResult(
        const planning_internal::Debug& debug_info,
        planning_internal::Debug* debug_chart)
{
...
void
OnLanePlanning::AddPartitionedTrajectory(
        const planning_internal::Debug& debug_info,
        planning_internal::Debug* debug_chart)
{
    // if open space info provider success run
...
void
OnLanePlanning::AddStitchSpeedProfile(planning_internal::Debug* debug_chart)
{
...
void
OnLanePlanning::AddPublishedSpeed(const ADCTrajectory& trajectory_pb,
        planning_internal::Debug* debug_chart)
{
    // if open space info provider success run
    if (!frame_->open_space_info().open_space_provider_success())
    ...
    for (const auto& point : trajectory_pb.trajectory_point())
    {
        auto* point_debug = speed_profile->add_point();
        point_debug->set_x(
                point.relative_time() + trajectory_pb.header().timestamp_sec());
...
void
OnLanePlanning::AddPublishedAcceleration(
